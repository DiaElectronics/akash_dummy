...
func TestEditUser(tt *testing.T) {
	...
	rulesSet.EXPECT().EditUserAccessManager(gomock.Any()).Return(true)
		mockRepo.EXPECT().EditUser(gomock.Any(), gomock.Any(), gomock.Any()).Return(testUser2, nil)
		b, err := a.EditUser(profile, testUser1.ID, testUser1)
		t.Nil(err)
			t.DeepEqual(testUser2, b)
}
...
func TestListUser(tt *testing.T) {
	...
	rulesSet.EXPECT().ListUserAccessManager(gomock.Any()).Return(true)
		mockRepo.EXPECT().ListUser(gomock.Any(), gomock.Any()).Return(testUsers, 2, []string{}, nil)
		b, count, _, err := a.ListUser(profile, listParams)
		t.Nil(err)
		t.Equal(2, count)
