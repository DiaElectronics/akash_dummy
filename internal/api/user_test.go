...
import (
	...
	user "demo/internal/api/restapi/client/user"
...
func TestGetUser(tt *testing.T) {
	...
	params := user.NewGetUserParams()
		...
		mockApp.EXPECT().GetUser(gomock.Any(), gomock.Any()).Return(appUser(testUser1), nil)

		t.Run("", func(tt *testing.T) {
			t := check.T(tt)
			geted, err := c.User.GetUser(params, cl.APIKeyAuth("Authorization", "header", sess))
			t.Nil(err)
			t.DeepEqual(geted.Payload, apiUser(appUser(testUser1)))
		})
}
...
func TestAddUser(tt *testing.T) {
	...
	params := user.NewAddUserParams()	
		...
		mockApp.EXPECT().AddUser(gomock.Any(), gomock.Any()).Return(appUser(testUser1), nil)
...
func TestEditUser(tt *testing.T) {
	...
	params := user.NewEditUserParams()
			params.Body.Data = testAddUser1
				params.Body.ID = testUser1.ID
		mockApp.EXPECT().EditUser(gomock.Any(), gomock.Any(), gomock.Any()).Return(appUser(testUser1), nil)
		t.Run("", func(tt *testing.T) {
			t := check.T(tt)
			geted, err := c.User.EditUser(params, cl.APIKeyAuth("Authorization", "header", sess))
			t.Nil(err)
				t.DeepEqual(geted.Payload, apiUser(appUser(testUser1)))
		})
}
...
func TestListUser(tt *testing.T) {
	...
	params := user.NewListUserParams()
		...
		mockApp.EXPECT().ListUser(gomock.Any(), gomock.Any()).Return(appUsers(testUsers), 2, []string{}, nil)

		t.Run("", func(tt *testing.T) {
			t := check.T(tt)
			list, err := c.User.ListUser(params, cl.APIKeyAuth("Authorization", "header", sess))
			t.Nil(err)
			t.DeepEqual(list.Payload.Items, apiUsers(appUsers(testUsers)))
		})
}
