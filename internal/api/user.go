...
func (svc *service) EditUser(params user.EditUserParams, profile interface{}) middleware.Responder {
	prof := profile.(*extauthapi.Profile)
		c, err := svc.app.EditUser(toAppProfile(prof), params.Body.ID, appUserAdd(params.Body.Data))
		switch {
		default:
		...
		case err == nil:
			...
			return user.NewEditUserOK().WithPayload(apiUser(c))
		}
}
func (svc *service) ListUser(params user.ListUserParams, profile interface{}) middleware.Responder {
	prof := profile.(*extauthapi.Profile)
		c, count, warnings, err := svc.app.ListUser(toAppProfile(prof), appListParams(params.Body))
		switch {
		default:
...
func appUser(a *models.User) *app.User {
	if a == nil {
		return nil
	}
	user := &app.User{}
			user.ID = a.ID
			user.Balance = types.NewDecimal(a.Balance)
			user.Name = a.Name
			user.Role = a.Role
...
func appUsers(apis []*models.User) []*app.User {
	apps := []*app.User{}
	for i := range apis {
		apps = append(apps, appUser(apis[i]))
	}
	return apps
}
